...
|---|---|---|---|
| Precision | 0.466 | 0.434 | +0.032 |
| Recall | 0.348 | 0.344 | +0.004 |
| mAP50 | 0.335 | 0.320 | +0.015 |
| mAP50-95 | 0.194 | 0.181 | +0.013 |

### 各类别 mAP50（val split，best.pt）

| 类别 | EXP-02 (P2) | EXP-01 (Baseline) | Delta |
|---|---|---|---|
| pedestrian | 0.391 | 0.343 | +0.048 |
| people | 0.301 | 0.263 | +0.038 |
| bicycle | 0.0967 | 0.083 | +0.0137 |
| car | 0.777 | 0.748 | +0.029 |
| van | 0.361 | 0.349 | +0.012 |
| truck | 0.286 | 0.301 | -0.015 |
| tricycle | 0.200 | 0.190 | +0.010 |
| awning-tricycle | 0.0956 | 0.091 | +0.0046 |
| bus | 0.457 | 0.472 | -0.015 |
| motor | 0.390 | 0.359 | +0.031 |
| **mAP50 均值** | **0.335** | **0.320** | **+0.015** |

### 推理速度与模型规模

| 指标 | EXP-02 (P2) | EXP-01 (Baseline) |
|---|---|---|
| 推理速度（ms/img，GPU） | 12.7 | ~3.2 |
| 参数量（M） | 2.66 | 2.62 |
| FLOPs（G） | 10.2 | 6.6 |
...
| 节点 | Epoch | mAP50 | 说明 |
|---|---|---|---|
| best.pt 保存 | 100 | 0.335 | 本次训练 `best.pt` 与最终 epoch 一致 |
| 训练结束 | 100 | 0.335 | `last.pt`/`best.pt` 均已导出 |

---

## 主要观察

- bicycle / awning-tricycle AP **没有显著提升**。bicycle 从 0.083 提升到 0.0967（+0.0137），awning-tricycle 从 0.091 提升到 0.0956（+0.0046），均明显低于预期。
- 整体 Recall **仅小幅提升**：0.344 → 0.348（+0.004），说明 P2 分支并未明显缓解漏检问题。
- 整体 mAP50 有一定提升：0.320 → 0.335（+0.015），但主要收益来自 pedestrian、people、car、motor 等类别，小目标专属收益不足。
- 推理代价明显增加。推理时间由约 3.2 ms 上升到 12.7 ms，FLOPs 从 6.6G 增至 10.2G，效率损失明显高于预期。
- 训练过程中多次出现 `CUDA OutOfMemoryError in TaskAlignedAssigner, using CPU`，说明在 `batch=32` 条件下，P2 分支显著增加了标签分配阶段的显存压力。虽然训练最终完成，但该现象意味着本配置的训练稳定性和效率都不理想。
- 从最终结果看，P2 分支对整体检测有轻度正增益，但**不满足“以较小代价显著提升极小目标 AP”这一设计目标**。
...
- EXP-02 证明了“单独增加 P2 分支”可以带来整体指标的小幅正增益，但增益强度有限。
- 该结构对 bicycle、awning-tricycle 等极小目标的提升不足，未达到预期的核心目标。
- 同时，P2 分支显著增加了训练与推理成本，尤其在 `batch=32` 下触发了 TaskAlignedAssigner 的显存回退问题。
- 因此，**P2 分支不能作为最终独立改进模块直接保留**，后续必须结合更高效的跨尺度融合、注意力增强或更稳健的小目标损失设计，才能释放其潜力。
